...
export const ApplyBox = () => {
    ...
    useEffect(() => {
        const appList = localStorage.getItem('documents')
        // Parse and filter by document type
        if (appList) {
            const parsedAppList = JSON.parse(appList)
            const filteredAppList = (parsedAppList).filter((doc: Document) => doc.type === 'APP_PKG')
    ...
    const getApplicationObject: () => Document | undefined = () => {
        return appList.find((app) => app.title === selectedAppID);
    }
            ...
            <div className="bg-white p-6 py-8 sm:p-8 rounded-lg">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <select
                        ...
                        value={getApplicationObject()?.title || ''}
